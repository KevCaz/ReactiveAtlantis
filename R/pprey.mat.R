    ...
    shiny::shinyApp(
        ...
        function(input, output, session) {
            ...
            spatial <- shiny::reactive({
                ## This piece of code needs to be updated,  there is too many relics and seections prone to error and redundancy
#               browser()
#                #gp.pred  <- with(sp.ov, sp.ov[which(Predator == input$pred && Prey == input$prey, arr.ind = TRUE)])
                gp.pred  <- dplyr::filter(over.tmp, .data$Predator == input$pred, .data$Prey == input$prey)
                input.layer <- as.character(ifelse(input$layer == 'Sediment', max(sp.ov$Layer, na.rm = TRUE), input$layer))
                pred.ad  <- dplyr::filter(sp.ov, .data$variable == input$pred, .data$Stage == 'Adult', .data$Layer == input.layer)
                pred.juv <- dplyr::filter(sp.ov, .data$variable == input$pred, .data$Stage == 'Juvenile', .data$Layer == input.layer)
                prey.ad  <- dplyr::filter(sp.ov, .data$variable == input$prey, .data$Stage == 'Adult', .data$Layer == input.layer)
                prey.juv <- dplyr::filter(sp.ov, .data$variable == input$prey, .data$Stage == 'Juvenile', .data$Layer == input.layer)
